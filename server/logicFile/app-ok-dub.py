...
DEFAULT_OPTIONS = {
    ...
    "aspect": "original",           # stored only - not applied yet
...
# Global fit (ai_recap only - see fit_voice_to_video) - the whole narration is
# allowed to speed up this much before the video itself is ever slowed down.
GLOBAL_MAX_TEMPO = 1.22
MIN_VIDEO_SPEED = 0.90
END_SAFETY_PAD_SEC = 0.15

# Dubbing's own adaptive fit bounds (see fit_global_timing) - separate from
# ai_recap's above since dubbing may now also speed the video UP (never
# just down) to close dead-silence gaps left when the voice underruns its
# scene. Voice speed is synthesized once at the fixed +30% baseline (rate=
# TTS_RATE) and only ever pushed faster from there via post-hoc tempo, up
# to this ceiling - it never goes slower than +30%.
...
class GeminiTranslator:
    ...
    def translate_narration(self, transcript_text, target_chars, prev_text, style_text):
        text = self._call(
            build_narration_prompt(transcript_text, target_chars, prev_text, style_text),
            json_mode=False,
        )
...
def synthesize_blocks(blocks, workdir, voice, rate):
    for i, b in enumerate(blocks):
        ...
        b["caption_chunks"] = ["".join(tok for tok in chunk) for chunk in chunks]
...
def append_smooth(track, seg, crossfade_ms=20):
    ...
    if cf <= 0:
        return track + seg
    return track.append(seg, crossfade=cf)
...
def fit_global_timing(blocks, raw_durations, video_duration, tempo_max,
    ...
    ai_recap's one-way fit_voice_to_video, dubbing may now also speed the
    video UP (never just down) to close dead-silence gaps.
...
NARRATION_PROMPT_TEMPLATE = """You are writing the Burmese voiceover narration for a TikTok movie/drama recap video.
Below is the full transcript of the video, in its original language, in order from start to finish.

Write ONE continuous, flowing Burmese recap narration for it - the way a Myanmar TikTok recap narrator would tell
the story out loud. Preserve the story and the meaning, not the exact wording - this is NOT a line-by-line or
word-for-word translation.

__STYLE__

Output plain prose only: no line numbers, no timestamps, no bullet points, no headings, no markdown fences - just
the narration text itself, as continuous Burmese prose.

Insert a space between each Burmese word (word-segmented) so it can later be split into timed subtitle chunks.

Target length: approximately __TARGET_CHARS__ characters. Get close to this length - that's how long the
narration needs to be to fit the video at a natural speaking pace. Do not pad with filler, and do not cut the
story short.
...
def build_narration_prompt(transcript_text, target_chars, prev_text, style_text):
    if prev_text:
        tail = prev_text[-300:]
        prev_context = (
            f'The narration so far already said (in Burmese): "...{tail}"\n'
            "Continue the story naturally from there - do not repeat it, do not restart."
        )
    else:
        prev_context = "This is the start of the narration."
    prompt = NARRATION_PROMPT_TEMPLATE.replace("__STYLE__", style_text)
...
def translate_transcript(words, language_code, video_duration, keys, style_text, length_mult):
    translator = GeminiTranslator(keys, GEMINI_MODEL)
    word_chunks = split_word_chunks(words, choose_num_chunks(len(words)))
    total_words = sum(len(c) for c in word_chunks) or 1
    target_total_chars = max(1, round(video_duration * SCENE_CPS * length_mult))
    ...
    for i, chunk in enumerate(word_chunks):
        ...
        if len(word_chunks) > 1:
            chunk_target = max(1, round(target_total_chars * (len(chunk) / total_words)))
        else:
            chunk_target = target_total_chars
        print(f"  translating chunk {i + 1}/{len(word_chunks)} "
              f"({len(chunk)} words, target~{chunk_target} chars)")
        translation = translator.translate_narration(chunk_text, chunk_target, prev_text, style_text)
...
def synthesize_sentences(sentences, workdir, voice, rate):
    ...
    for i, text in enumerate(sentences):
        ...
        asyncio.run(_synth(text, raw_mp3, voice, rate))
        trimmed_mp3 = workdir / f"sent_{i:03d}.mp3"
        trim_silence(raw_mp3, trimmed_mp3)
        paths.append(trimmed_mp3)
        print(f"    sentence {i}: {ffprobe_duration(trimmed_mp3):.2f}s  {text[:50]!r}")
    return paths


def build_continuous_track(sentence_paths, out_wav):
    track = AudioSegment.empty()
    placements = []
    P = 0.0
    for path in sentence_paths:
        dur = ffprobe_duration(path)
        placements.append((P, dur))
        clip = AudioSegment.from_file(path)
        track = append_smooth(track, clip)
        P += dur
    track.export(out_wav, format="wav")
    return P, placements


def fit_voice_to_video(voice_duration, video_duration):
    """Fit happens once, at the end, against the finished track - never per
    line. If it's long: speed the voice up to 1.22x max, then slow the video
    to 0.90x at worst. If there's still a gap after both, that's fine - the
    video's last frame gets padded to cover it. If it's short: just pad
    silence at the end."""
    tempo = 1.0
    video_speed = 1.0
    stretched = voice_duration

    if voice_duration > video_duration + 1e-6:
        tempo = min(GLOBAL_MAX_TEMPO, voice_duration / video_duration)
        stretched = voice_duration / tempo
        if stretched > video_duration + 1e-6:
            video_speed = max(MIN_VIDEO_SPEED, video_duration / stretched)

    new_video_duration = video_duration / video_speed
    final_duration = max(new_video_duration, stretched) + END_SAFETY_PAD_SEC
    return tempo, video_speed, new_video_duration, final_duration


def apply_global_tempo_and_pad(naive_wav, tempo, final_duration, workdir, out_wav):
    if abs(tempo - 1.0) > 1e-4:
        sped_path = workdir / "track_tempo.wav"
        run([
            FFMPEG_BIN, "-y", "-i", str(naive_wav),
            "-filter:a", f"atempo={tempo}",
            str(sped_path),
        ], capture_output=True)
        src = sped_path
    ...
    final_ms = int(round(final_duration * 1000))
    if len(audio) < final_ms:
        audio += AudioSegment.silent(duration=final_ms - len(audio))
    elif len(audio) > final_ms:
        audio = audio[:final_ms]
...
def build_blur_stage(input_label, box, index, width, height):
    ...
    the same spot. `input_label` (e.g. "[vconcat]" or "[0:v]") is read
    twice here (once as the crop source, once as the overlay base) -
    ffmpeg auto-splits a raw INPUT stream referenced twice, but an
    intermediate labeled pad (like the crop below, reused for both the
    content branch and the mask blend) needs an explicit `split`.
    ...
    stage = (
        f"{input_label}crop={w}:{h}:{x}:{y}[{crop_label}];"
        ...
        f"{input_label}[{merged_label}]overlay={x}:{y}[{ov_label}]"
    )
    return stage, f"[{ov_label}]"


def build_pre_filters(flip_video, auto_color):
    """Flip and auto-color are applied ONCE to the raw source, before any
    trim/blur/subtitle stage - so a flip also mirrors what the blur boxes
    cover (their x is pre-mirrored by the caller to compensate) and a
    burned-in caption never itself gets flipped or color-shifted."""
    parts = []
    if flip_video:
        parts.append("hflip")
    if auto_color:
...
def mux_video_single(input_video, audio_wav, ass_path, video_speed, final_duration, video_duration,
                      blur_boxes, width, height, flip_video, auto_color, out_path):
    """Shared by ai_recap (continuous track, see fit_voice_to_video) and
    dubbing (per-block track laid out by fit_global_timing) - both now
    ...
    stages, cur = build_pre_filters(flip_video, auto_color)
...
def mux_subtitle_only(input_video, ass_path, blur_boxes, width, height, flip_video, auto_color, out_path):
    ...
    pre_stages, src_label = build_pre_filters(flip_video, auto_color)
...
def process_dubbing(input_video, workdir, words, lang, video_duration, width, height,
                     ...
                     sub, blur_boxes, flip_video, auto_color):
    ...
    mux_video_single(input_video, audio_wav, ass_path, video_speed, total_voice_duration, video_duration,
                      blur_boxes, width, height, flip_video, auto_color, out_video)
...
def process_ai_recap(input_video, workdir, words, lang, video_duration, width, height,
                      out_video, out_srt, gemini_keys, voice, rate, style_text, length_mult,
                      sub, blur_boxes, flip_video, auto_color):
    print("[4/7] translating the whole transcript into one flowing narration with Gemini")
    narration = translate_transcript(words, lang, video_duration, gemini_keys, style_text, length_mult)
    if not narration.strip():
        raise RuntimeError("translation returned no narration text")
    print(f"      narration length: {len(narration)} chars "
          f"(target was ~{round(video_duration * SCENE_CPS * length_mult)})")
    ...
    print(f"      split into {len(sentences)} sentences for TTS")
    sentence_paths = synthesize_sentences(sentences, workdir, voice, rate)

    print("[6/7] assembling continuous voice track and fitting to video")
    naive_wav = workdir / "naive_track.wav"
    voice_duration, placements = build_continuous_track(sentence_paths, naive_wav)
    tempo, video_speed, adj_video_dur, final_duration = fit_voice_to_video(voice_duration, video_duration)
    print(f"      voice duration={voice_duration:.2f}s, video duration={video_duration:.2f}s")
    print(f"      tempo={tempo:.4f} video_speed={video_speed:.4f} final_duration={final_duration:.2f}s")
    if abs(tempo - 1.0) > 1e-4:
        placements = [(s / tempo, d / tempo) for s, d in placements]

    audio_wav = workdir / "final_audio.wav"
    apply_global_tempo_and_pad(naive_wav, tempo, final_duration, workdir, audio_wav)

    print("[7/7] writing subtitles from the actual synthesized voice, then muxing")
    entries = build_subtitle_entries(sentences, placements)
    ass_path = write_subtitle_outputs(entries, out_srt, width, height, workdir, sub)

...
// 3. ADD MISSING STATES (error, finalVideoUrl)
content = content.replace(
  "const [error, setError] = useState('');",
  "const [error, setError] = useState('');\n  const [finalVideoUrl, setFinalVideoUrl] = useState('');"
);

// 4. ADD GOOGLE SUCCESS HANDLER & AUTO PROCESS HANDLER
const insertPoint = content.indexOf("const handleFileChange =");
...
const handlers = `
  ...
  const handleAutoProcess = async () => {
    ...
    try {
      ...
      const extractRes = await axios.post('http://localhost:5001/api/step1-extract', formData, { headers: { 'Content-Type': 'multipart/form-data', Authorization: \`Bearer \${token}\` } });
      ...
      const translateRes = await axios.post('http://localhost:5001/api/step2-translate', { utterances: extractedUtterances }, { headers: { Authorization: \`Bearer \${token}\` } });
      ...
      const ttsRes = await axios.post('http://localhost:5001/api/step3-tts', { translatedUtterances, voice: selectedVoice }, { headers: { Authorization: \`Bearer \${token}\` } });
...
content = content.slice(0, insertPoint) + handlers + content.slice(insertPoint);

// 5. ADD FULL PAGE LOGIN LOCK & AUTO MODE RENDER
const mainReturnRegex = /return \(\s*<div className="min-h-screen bg-\[#0F172A\] flex flex-col font-sans relative overflow-hidden">/;
...
content = content.replace(mainReturnRegex, loginAndAutoMode);

// We also need to add back the Navbar for Free users, but since the user is premium/admin anyway, they will see the new UI.
// But let's restore the Free user navbar!
const freeNavbarRegex = /<h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600">\s*Recap Studio\s*<\/h1>\s*<\/div>/;
...
content = content.replace(freeNavbarRegex, userUI);

fs.writeFileSync('client/src/App.jsx', content);
console.log('App.jsx fully restored and safely fixed!');
